...
class IntegratedVectorizerStrategy(Strategy):
    """
    A strategy for ingesting and vectorizing documents using Azure AI Search's integrated vectorization capabilities.
    This approach uses Azure AI Search's built-in vector generation rather than computing embeddings separately.
    
    The strategy handles:
    - Document ingestion into blob storage
    - Configuration of search index with vector search capabilities
    - Setup of document processing pipeline with chunking and embedding generation
    - Management of indexer connections between blob storage and search index
    """
    ...
    def __init__(
        ...
        """
        Initialize the integrated vectorizer strategy.

        Args:
            list_file_strategy: Strategy for listing input files to process
            blob_manager: Manager for blob storage operations
            search_info: Contains search service configuration and credentials
            embeddings: Azure OpenAI service for embedding generation
            subscription_id: Azure subscription identifier
            search_service_user_assigned_id: Managed identity for search service
            document_action: Specifies whether to add or remove documents
            search_analyzer_name: Custom analyzer for text processing
            use_acls: Whether to enable access control lists
            category: Optional category label for documents
        """
    ...
    async def create_embedding_skill(self, index_name: str):
        """
        Creates a search skillset that handles document chunking and embedding generation.
        
        Args:
            index_name: Name of the target search index
            
        Returns:
            SearchIndexerSkillset configured with splitting and embedding skills
        """
    ...
    async def setup(self):
        """
        Sets up required search infrastructure:
        - Creates/updates search index with vector search capabilities
        - Configures data source connection to blob storage
        - Creates skillset for document processing
        """
    ...
    async def run(self):
        """
        Executes the document processing strategy:
        - For DocumentAction.Add: Uploads documents to blob storage
        - For DocumentAction.Remove: Removes specified documents
        - For DocumentAction.RemoveAll: Removes all documents
        Then creates and runs an indexer to process the documents
        """
